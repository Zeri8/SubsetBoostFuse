...
class Tokenize(nn.Module):
    ...
    def forward(self, x):
        flair_intra_x, t1ce_intra_x, t1_intra_x, t2_intra_x = torch.chunk(x, self.num_modals, dim=1) #(B, 512, 8, 8, 8)
        multimodal_token_x = torch.cat((flair_intra_x.permute(0, 2, 3, 4, 1).contiguous().view(x.size(0), -1, self.dims), #(B, 512, 512)
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x2 = self.e2_c1(x1)
        x2 = x2 + self.e2_c3(self.e2_c2(x2)) #(B, 64, 64, 64, 64)
...
class Decoder_fuse(nn.Module):
    ...
    def forward(self, x1, x2, x3, x4, x5, mask=None):
        ...
        de_x1 = self.d1_out(self.d1_c2(de_x2))      #(B, 8, 128, 128, 128)
...
class Model(nn.Module):
    def __init__(
        ...
        ########### InterFormer
        self.mamba_fusion_layer = MambaFusionLayer(dim=transformer_basic_dims, num_tokens_fused_representation=patch_size**3)
    ...
    def forward(self, x, mask):
        ...
        ########### IntraFormer
        flair_token_x5 = self.flair_encode_conv(flair_x5).permute(0, 2, 3, 4, 1).contiguous().view(x.size(0), -1, transformer_basic_dims) #(B, 512, 512)
        ...
        flair_intra_x5 = flair_intra_token_x5.view(x.size(0), patch_size, patch_size, patch_size, transformer_basic_dims).permute(0, 4, 1, 2, 3).contiguous() #(B, 512, 8, 8, 8)
        ...
        x1 = self.masker(torch.stack((flair_x1, t1ce_x1, t1_x1, t2_x1), dim=1), mask) #Bx4xCxHWZ = (B, 32, 128, 128, 128)
        ...
        x5_intra = self.masker(torch.stack((flair_intra_x5, t1ce_intra_x5, t1_intra_x5, t2_intra_x5), dim=1), mask) #(1, 2048, 8, 8, 8)

        if self.mamba_skip:
            #x1 = self.tokenize[-5](x1) #(B, 128**3, 8)*4
            #x1 = self.mamba_fusion_layers[-5](x1) # (B, 128**3, 8)
            #x1 = x1.view(x.size(0), input_patch_size, input_patch_size, input_patch_size, basic_dims).permute(0, 4, 1, 2, 3).contiguous() #(B, 8, 128, 128, 128)

            x2 = self.tokenize[-4](x2) # (B, 64**3, 16)*4
            x2 = self.mamba_fusion_layers[-4](x2) # (B, 64**3, 16)
            x2 = x2.view(x.size(0), input_patch_size//2, input_patch_size//2, input_patch_size//2, basic_dims*2).permute(0, 4, 1, 2, 3).contiguous() #(B, 16, 64, 64, 64)

            x3 = self.tokenize[-3](x3) # (B, 32**3, 32)*4
            x3 = self.mamba_fusion_layers[-3](x3) # (B, 32**3, 32)
            x3 = x3.view(x.size(0), input_patch_size//4, input_patch_size//4, input_patch_size//4, basic_dims*4).permute(0, 4, 1, 2, 3).contiguous() #(B, 32, 32, 32, 32)

            x4 = self.tokenize[-2](x4) # (B, 16**3, 64)*4
            x4 = self.mamba_fusion_layers[-2](x4) # (B, 16**3, 64)
            x4 = x4.view(x.size(0), input_patch_size//8, input_patch_size//8, input_patch_size//8, basic_dims*8).permute(0, 4, 1, 2, 3).contiguous() #(B, 64, 16, 16, 16)
        #######        

        ########### InterFormer
        # flair_intra_x5, t1ce_intra_x5, t1_intra_x5, t2_intra_x5 = torch.chunk(x5_intra, num_modals, dim=1) #(B, 512, 8, 8, 8)
        # multimodal_token_x5 = torch.cat((flair_intra_x5.permute(0, 2, 3, 4, 1).contiguous().view(x.size(0), -1, transformer_basic_dims), #(B, 512, 512)
        ...
        #                                 ), dim=1)
        # multimodal_pos = torch.cat((self.flair_pos, self.t1ce_pos, self.t1_pos, self.t2_pos), dim=1) #(B, 2048, 512)
